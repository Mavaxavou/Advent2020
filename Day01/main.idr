...
split : Ord t => List t -> (List t, List t)
split xs = onFst reverse $ splitAt (divCeilNZ (length xs) 2 absurd) $ sort xs
...
lookForGoal : (List Nat, List Nat) -> (goal : Nat) -> Result
lookForGoal ([], us) goal = ExhaustLeft
lookForGoal (ls, []) goal = ExhaustRight
lookForGoal (l :: ls, u :: us) goal with (compare (l + u) goal)
  | EQ = Found (l * u)
  | LT = lookForGoal (l :: ls, us) goal
  | GT = lookForGoal (ls, u :: us) goal

compute : List Nat -> Nat -> Maybe Nat
compute [] goal = Nothing
compute inputs goal =
  let (ls, us) = split inputs in
  case lookForGoal (ls, us) goal of
    Found res => Just res
    ExhaustLeft  => compute ls goal
    ExhaustRight => compute us goal

forThree : List Nat -> Nat -> Maybe Nat
forThree inputs goal = aux inputs where
  aux [] = Nothing
  aux (elt :: elts) with (isLTE elt goal)
    | Yes prf =
      case compute inputs (goal - elt) of
        Nothing  => aux elts
        Just res => Just (elt * res)
    | No cntr = aux elts
...
main : IO ()
main = do putStrLn $ show $ forThree inputs 2020
